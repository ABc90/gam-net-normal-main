...
class PCATrans(object):
    ...
    def __call__(self, data):
        ...
        if 'gf_normal_center' in data:
            data['gf_normal_center'] = torch.matmul(data['gf_normal_center'], trans)
            NN = data['gf_normal_center']
            data['z-axis-direction-gf'] = np.dot(NN,[0,0,1])
...
class PointCloudDataset(Dataset):
    ...
    def get_data(self, cur_sets):
        for s in tqdm(cur_sets, desc='Loading data'):
            ...
            if os.path.exists(os.path.join(self.data_dir, 'modify/'+ s + '.mnormals')):
                modify_nor = load_data(filedir=self.data_dir+'/modify/', filename=s + '.mnormals', dtype=np.float32)
            else:
                modify_nor = nor
            # 231017_211111_FamousShape， 231007_140818_PCPNet,240117_085851_SceneNN
            if os.path.exists('/media/junz/volume1/junz/Neural_GF/log/231017_211111_FamousShape，/test_20000/pred_normal/' + s +'_normal.npy'):
                gf_nor = np.load('/media/junz/volume1/junz/Neural_GF/log/231017_211111_FamousShape，/test_20000/pred_normal/'+s +'_normal.npy')
            else:
                gf_nor = nor

            if os.path.exists(os.path.join(self.data_dir, 'modify/'+ s + '.mdensity')):
                noise_density = load_data(filedir=self.data_dir+'/modify/', filename=s + '.mdensity', dtype=np.float32)
            else:
                noise_density = np.ones((pcl.shape[0]))

            if os.path.exists(os.path.join(self.data_dir, 'modify/'+ s + '.mconf')):
                confidence = load_data(filedir=self.data_dir+'/modify/', filename=s + '.mconf', dtype=np.float32)
            else:
...
class PatchDataset(Dataset):
    ...
    def make_patch(self, pcl, kdtree=None, nor=None, conf=None, density=None, mnor = None, gf_normal = None, query_idx=None, patch_size=1):
        ...
        if gf_normal is not None:
            nor_gf = gf_normal[pat_idx,:]
        return pcl_pat, nor_pat, mnor_pat, conf_pat, denstiy_pat, dist_max, seed_pnts, nor_gf
    ...
    def __getitem__(self, idx):
        ...
        pcl_pat, normal_pat, mnor_pat, conf_pat, denstiy_pat, dist_max, seed_pnts, nor_gf= self.make_patch(pcl=shape_data['pcl'],
                                                                              kdtree=shape_data['kdtree'],
                                                                              nor=shape_data['normal'],
                                                                              conf =  shape_data['confidence'],
                                                                              density = shape_data['density'],
                                                                              mnor = shape_data['mnormal'],
                                                                              gf_normal = shape_data['gf_normal'],
                                                                              query_idx=query_idx,
        ...
        data = {'name': shape_data['name'],
                ...
                'normal_center': torch.from_numpy(shape_data['normal'][int(query_idx), :]).float(),
                'mnormal_pat': torch.from_numpy(mnor_pat).float(),
                'mnormal_center': torch.from_numpy(shape_data['mnormal'][int(query_idx), :]).float(),
                'conf_pat': torch.from_numpy(np.expand_dims(conf_pat,axis=1)).float(),
                'conf_center': torch.from_numpy(np.expand_dims(shape_data['confidence'][int(query_idx)], axis=0)).float(),
                'density_pat': torch.from_numpy(np.expand_dims(denstiy_pat, axis=1)).float(),
                'density_center': torch.from_numpy(np.expand_dims(shape_data['density'][int(query_idx)], axis=0)).float(),
                'pcl_gf_normal':torch.from_numpy(nor_gf).float(),
                'gf_normal_center':torch.from_numpy(shape_data['gf_normal'][int(query_idx), :]).float(),
            }
