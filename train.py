...
from dataset_modify import PointCloudDataset, PatchDataset, RandomPointcloudPatchSampler
...
def parse_arguments():
    ...
    parser.add_argument('--data_set', type=str, default='PCPNet', choices=['PCPNet', 'SceneNN', 'Semantic3D', 'KITTI_sub', 'FamousShape'])
...
def train(epoch):
    ...
    for train_batchind, batch in enumerate(train_dataloader, 0):
        ...
        mnormal_pat =  batch['mnormal_pat'].to(_device)  
        mnormal_center = batch['mnormal_center'].to(_device).squeeze()  # (B, 3)   
        conf_pat = batch['conf_pat'].to(_device)  
        conf_center = batch['conf_center'].to(_device)  
        nconf_pat = batch['nconf_pat'].to(_device)  
        nconf_center = batch['nconf_center'].to(_device)  
        density_pat = batch['density_pat'].to(_device)  
        density_center = batch['density_center'].to(_device)  
        ...
        # pcl_pat_t = batch['pcl_pat']
        # noise_density_pat_t = batch['noise_density_pat']
        # pt = o3d.geometry.PointCloud()
        # pt.points = o3d.utility.Vector3dVector(pcl_pat_t[1,...])
        # colors = plt.get_cmap("hot")(noise_density_pat_t[1,...])
        # pt.colors = o3d.utility.Vector3dVector(colors[:, 0,:3])      
        # o3d.visualization.draw_geometries([pt])



        ### Forward
        pred_point, weights, pred_neighbor ,qstn, _,_, _,_, _= model(pcl_pat, pcl_sample=pcl_sample,d=direction)
        # pred_point, weights, pred_neighbor ,qstn= model(pcl_pat, pcl_sample=pcl_sample,d=direction)

        # loss, loss_tuple = model.get_loss_old(q_target=normal_center, q_pred=pred_point,
        #                                     ne_target=normal_pat, ne_pred=pred_neighbor,
        #                                     pred_weights=weights, pcl_in=pcl_pat, d = direction,
        #                                 )

        #mnormal_center,normal_center
        loss, loss_tuple = model.get_loss(q_target=mnormal_center, q_pred=pred_point,
                                            ne_target=normal_pat, ne_pred=pred_neighbor,
                                            pred_weights=weights, pcl_in=pcl_pat, 
                                            qstn = qstn,
                                            q_confidence = conf_center,
                                            n_confidence = nconf_center,
                                            ne_confidence = conf_pat,)
